...
/**
 * Port of Thaumcraft 6 Aspect API (M1).
 * Research scanning hook intentionally deferred to M5.
 */
public class Aspect {

    public static final HashMap<Integer, Aspect> mixList = new HashMap<>();
    ...
    private static final ArrayList<Aspect> primals = new ArrayList<>();
    private static final ArrayList<Aspect> compounds = new ArrayList<>();
    ...
    public Aspect(String tag, int color, Aspect[] components) {
        this(tag, color, components, aspectTexture(tag), 1);
    }

    public Aspect(String tag, int color, Aspect[] components, int blend) {
        this(tag, color, components, aspectTexture(tag), blend);
    }
    ...
    private static ResourceLocation aspectTexture(String tag) {
        return new ResourceLocation(Arcana.MODID, "textures/aspects/" + tag.toLowerCase(Locale.ROOT) + ".png");
    }
    ...
    public String getName() {
        ...
        return tag.substring(0, 1).toUpperCase(Locale.ROOT) + tag.substring(1).toLowerCase(Locale.ROOT);
    }

    public String getLocalizedDescription() {
        return Language.getInstance().getOrDefault("arcana.aspect." + tag);
    }
